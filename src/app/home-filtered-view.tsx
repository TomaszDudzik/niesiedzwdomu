...
function HomeSectionSubmissionCta({
  ...
  return (
    <Link
      href={href}
      className="group mb-4 flex w-full items-center gap-3 rounded-2xl border border-[#7FCBC6] bg-[#D6EFEC] px-3 py-3 shadow-[0_8px_20px_-16px_rgba(10,191,163,0.09)] transition-all duration-200 hover:border-[#5BBFBA] hover:bg-[#C0E8E5]"
    >
      <div className="h-10 w-1 shrink-0 rounded-full bg-primary" />
      <div className="min-w-0 flex-1">
        <p className="text-[12px] font-semibold leading-4 text-foreground sm:text-[13px] lg:text-[14px]">{title}</p>
        <p className="mt-1 text-[10px] leading-4 text-muted sm:text-[11px] lg:text-[12px]">{description}</p>
      </div>
      <span className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap rounded-full border border-primary/20 bg-white/85 px-2.5 py-1.5 text-[10px] font-semibold text-primary shadow-[0_8px_18px_-12px_rgba(240,75,26,0.4)] transition-all duration-200 group-hover:border-primary/40 group-hover:bg-primary group-hover:text-white sm:px-3 sm:text-[11px]">
        {buttonLabel}
...
export function HomeFilteredView({ events, places, camps, activities }: HomeFilteredViewProps) {
  ...
  return (
    <div className="container-page pt-6 pb-12">
      {/* Mobile top bar */}
      <div className="lg:hidden rounded-xl border border-[#7FCBC6] bg-[#D6EFEC] p-3 mb-4 flex items-center gap-2 shadow-[var(--shadow-soft)]">
        <button
          onClick={() => setFiltersOpen(!filtersOpen)}
          className={cn("inline-flex items-center gap-2 px-3 py-2 rounded-lg text-[12px] font-semibold border-2 transition-all duration-200",
            filtersOpen || hasActiveFilters ? "bg-primary text-primary-foreground border-primary shadow-[0_4px_14px_rgba(240,75,26,0.38)]" : "bg-primary/8 text-primary border-primary/25 hover:bg-primary/15")}
        >
        ...
        <div className="relative flex-1">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-primary/50" />
          <input type="text" placeholder="Szukaj miejsc, wydarzeń, kolonii..." value={search} onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-8 pr-3 py-2 rounded-lg border border-primary/20 bg-background text-[13px] text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/25 focus:border-primary/50 shadow-[inset_0_1px_4px_rgba(45,41,38,0.06)] transition-all duration-200" />
        </div>
      </div>

      {/* Mobile filters dropdown */}
      {filtersOpen && (
        <div className="lg:hidden rounded-xl border border-[#7FCBC6] bg-[#D6EFEC] p-3 mb-4 space-y-2.5">
      ...
      <div className="lg:flex lg:gap-6 lg:items-start">
        ...
        <aside className="hidden lg:block w-56 shrink-0">
          <div className="rounded-xl border border-[#7FCBC6] bg-[#D6EFEC] p-3 space-y-3 shadow-[var(--shadow-soft)]">
            <div className="relative">
              <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-primary/50" />
              <input type="text" placeholder="Szukaj..." value={search} onChange={(e) => setSearch(e.target.value)}
                className="w-full pl-8 pr-3 py-2 rounded-lg border border-[#7FCBC6] bg-white text-[12px] text-foreground placeholder:text-[#2D8A85] focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all duration-200" />
            </div>

            <div className="border-t border-border" />

            <FilterSection title={<span className="text-[10px] font-bold text-[#0F5955] uppercase tracking-widest">Grupa</span>} defaultCollapsed={!filtersOpenDesktop}>
              <div className="flex flex-col gap-0.5">
                {typeLevel2Options.map((option) => {
                  const selected = activeTypeLevel2.includes(option.value);
                  return (
                    <button key={option.value} onClick={() => toggleTypeLevel2(option.value)}
                      className={cn("flex items-center gap-1.5 px-2 py-1 rounded-lg text-[11px] font-medium text-left transition-all duration-200",
                        selected ? "bg-primary text-primary-foreground border border-primary shadow-[0_2px_8px_rgba(10,191,163,0.14)]" : "text-foreground bg-[#E2F6F4] border border-[#7FCBC6] hover:border-[#2D8A85] hover:text-[#0F5955] hover:bg-[#C0E8E5]")}>
                      <span>{option.icon}</span>
                      <span className="flex-1">{option.label}</span>
                      {selected && <Check size={10} />}
                      <span className="text-[9px] opacity-50 tabular-nums">{option.count}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <FilterSection title={<span className="text-[10px] font-bold text-[#0F5955] uppercase tracking-widest">Typ</span>} defaultCollapsed={!filtersOpenDesktop}>
              <div className="flex flex-col gap-0.5">
                {typeOptions.map((option) => {
                  const selected = activeTypes.includes(option.value);
                  return (
            ...
            <FilterSection title={<span className="text-[10px] font-bold text-[#0F5955] uppercase tracking-widest">Wiek</span>} defaultCollapsed={!filtersOpenDesktop}>
              <div className="flex flex-col gap-0.5">
                {ageOptions.filter((group) => group.count > 0 || activeAgeGroup === group.key).map((group) => (
                  <button key={group.key} onClick={() => setActiveAgeGroup(activeAgeGroup === group.key ? null : group.key)}
                    className={cn("flex items-center gap-1.5 px-2 py-1 rounded-lg text-[11px] font-medium text-left transition-all duration-200",
                    ...
                    <span>{group.icon}</span>
                    <span>{group.label}</span>
                    <span className="ml-auto text-[9px] opacity-50 tabular-nums">{group.count}</span>
                  </button>
                ))}
              </div>
            </FilterSection>

            <FilterSection title={<span className="text-[10px] font-bold text-[#0F5955] uppercase tracking-widest">Kategoria</span>}>
              <div className="flex flex-col gap-0.5">
                {categoryOptions.map((option) => {
                  const selected = activeCategories.includes(option.value);
                  return (
            ...
            <FilterSection title={<span className="text-[10px] font-bold text-[#0F5955] uppercase tracking-widest">Dzielnica</span>}>
              <div className="flex flex-col gap-0.5">
                {districtOptions.map((option) => {
                  const selected = activeDistricts.includes(option.value);
                  return (
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <div className="rounded-xl border border-[#7FCBC6] bg-[#D6EFEC] px-2.5 py-2 shadow-[var(--shadow-card)]">
