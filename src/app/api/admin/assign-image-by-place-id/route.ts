...
export async function POST(request: NextRequest) {
  ...
  if (!id || !placeId) {
    return NextResponse.json({ ok: false, error: "id and place_id are required" }, { status: 400 });
  }

  const scriptPath = path.join(process.cwd(), "scripts", "assign_place_image.py");
  const pythonExecutable = resolvePythonExecutable();
  const command = `${pythonExecutable} ${quoteArg(scriptPath)} --id ${quoteArg(id)} --place-id ${quoteArg(placeId)}`;
