...
export default function CalendarPage() {
  ...
  const selectedEvents = useMemo(() => getEventsForDate(publishedEvents, selectedDate), [selectedDate]);

  const eventCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (let d = 1; d <= daysInMonth; d++) {
      const events = getEventsForDate(publishedEvents, new Date(currentYear, currentMonth, d));
      if (events.length > 0) counts[d] = events.length;
    }
    return counts;
                ...
                {selectedEvents.map((event) => (
                  <Link key={event.id} href={`/wydarzenia/${event.slug}`}
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-muted">
                      ...
                      <span className="flex items-center gap-1"><MapPin size={10} />{event.venue_name}</span>
                    </div>
