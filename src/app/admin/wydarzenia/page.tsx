...
function AdminCanonicalEventsPanel() {
  ...
  const openPromptModal = async () => {
    ...
    try {
      const saved = localStorage.getItem("admin_prompt_urls_wydarzenia");
      if (saved) {
              ...
              <>
                ...
                <div className="flex justify-between gap-2 px-5 py-4 border-t border-border">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        try { localStorage.setItem("admin_prompt_urls_wydarzenia", JSON.stringify({ rows: promptUrlRows, statuses: promptUrlStatuses })); } catch {}
