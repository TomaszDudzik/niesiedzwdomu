...
export function Header({ counts }: { counts?: NavCounts }) {
  ...
  const sectionBadge = pathname.startsWith("/miejsca")
    ? { label: "Miejsca", text: "#3A8C3F" }
    : pathname.startsWith("/wydarzenia")
      ? { label: "Wydarzenia", text: "#F5C200" }
      : pathname.startsWith("/kolonie")
        ? { label: "Kolonie", text: "#e60100" }
        : pathname.startsWith("/zajecia")
          ? { label: "Zajęcia", text: "#8B5CF6" }
          : null;
    ...
    >
      ...
      <div className="container-page">
        <div className="relative flex h-[68px] items-center gap-4 lg:gap-6">
          ...
          <div className="shrink-0 inline-flex items-end whitespace-nowrap">
            {/* Logo */}
            <Link
              href="/"
              className="inline-flex items-end self-end whitespace-nowrap"
              aria-label="NieSiedzWDomu"
            >
              <span style={{ fontFamily: "var(--font-pacifico)", fontSize: "clamp(22px, 4vw, 30px)", color: "#ffffff", letterSpacing: "-0.01em", lineHeight: 1 }}>
            ...
            {sectionBadge && (
              <span
                className="ml-4 inline-flex items-end self-end font-bold leading-none tracking-[-0.01em] text-white/90"
                style={{ color: sectionBadge.text, fontSize: "clamp(16px, 2.8vw, 22px)" }}
              >
                <span className="mr-3 text-[1.12em] font-semibold leading-none text-white">|</span>
                {sectionBadge.label}
              </span>
            )}
          </div>
