#!/usr/bin/env python3
"""
Assign a randomly chosen image to one activity by activity_id.

Flow:
- find ALL source images whose filename starts with activity_id under PHOTOS_DIR
...
- update target row in public.activities

Usage:
  python scripts/assign_activity_image.py --id <activity_row_id> --activity-id ACT-000123
"""
...
PHOTOS_DIR = Path(r"C:\Users\dudzi\OneDrive\NieSiedzWDomu\photos\dzieci\zajecia")
...
def _find_source_images(activity_id: str) -> list[Path]:
...
def _main() -> int:
    parser = argparse.ArgumentParser(description="Assign image to activity by activity_id")
    parser.add_argument("--id", required=True, help="UUID row id in activities table")
    parser.add_argument("--activity-id", required=True, help="Human activity id like ACT-000123")
    ...
    sources = _find_source_images(args.activity_id)
    if not sources:
        print(json.dumps({"ok": False, "error": f"No source images found for {args.activity_id}"}), file=sys.stderr)
        return 1
    ...
    cover_path = f"dzieci/zajecia/{storage_dir}/{cover_name}" if storage_dir else f"dzieci/zajecia/{cover_name}"
    thumb_path = f"dzieci/zajecia/{storage_dir}/{thumb_name}" if storage_dir else f"dzieci/zajecia/{thumb_name}"
    ...
    response = client.table("activities").update(update).eq("id", args.id).execute()

    if response is None:
        print(json.dumps({"ok": False, "error": "Failed to update activity"}), file=sys.stderr)
        return 1
